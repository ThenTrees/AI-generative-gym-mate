...
class ChatbotService {
  ...
  async processMessage({
    ...
    try {
      ...
      const quickResponse = this.checkQuickResponse(
        message,
        context?.userProfile
      );
      if (quickResponse) {
      ...
      const conversationHistory =
        this.getConversationFromMemory(activeConversationId) || [];

      // Analyze message intent
      const intent = this.analyzeMessageIntent(message);
      logger.info(
        `Message intent analyzed: "${intent}" for message: "${message}"`
      );
      ...
      if (exerciseAnalysis?.isExerciseQuery) {
        logger.info(
          `Exercise query detected! Found ${exerciseAnalysis.exercises.length} relevant exercises`
        );
      }

      // ✅ NEW: Analyze nutrition intent and search for relevant foods
      const nutritionAnalysis = await this.analyzeNutritionIntent(
        message,
        context?.userProfile,
      ...
      if (nutritionAnalysis?.isNutritionQuery) {
        logger.info(
          `Nutrition query detected! Found ${nutritionAnalysis.foods.length} relevant foods`
        );
      }
      ...
      if (
        exerciseAnalysis?.isExerciseQuery &&
        exerciseAnalysis.exercises.length > 0
      ) {
        ...
        logger.info(
          `✅ Added ${response.exercises.length} structured exercises to response`
        );
        logger.info(
          `📊 Response structure: hasExercises=${response.hasExercises}, exerciseCount=${response.exercises.length}, exerciseType=${response.exerciseType}`
        );
      } else {
        logger.info(
          `⚠️ No exercises added - isExerciseQuery: ${
            exerciseAnalysis?.isExerciseQuery
          }, count: ${exerciseAnalysis?.exercises?.length || 0}`
        );
      }

      // ✅ NEW: Add food data to response if available
      if (
        nutritionAnalysis?.isNutritionQuery &&
        nutritionAnalysis.foods.length > 0
      ) {
        ...
        logger.info(
          `✅ Added ${response.foods.length} structured foods to response`
        );
        logger.info(
          `📊 Response structure: hasFoods=${response.hasFoods}, foodCount=${response.foods.length}, category=${response.foodCategory}`
        );
      } else {
        logger.info(
          `⚠️ No foods added - isNutritionQuery: ${
            nutritionAnalysis?.isNutritionQuery
          }, count: ${nutritionAnalysis?.foods?.length || 0}`
        );
      }
  ...
  private analyzeMessageIntent(message: string): string {
    ...
    if (
      lowerMessage.match(
        /lịch tập.*thực đơn|thực đơn.*lịch tập|plan.*nutrition|nutrition.*plan|tập.*ăn|ăn.*tập/
      )
    ) {
      if (
        lowerMessage.match(
          /tăng cơ|tang co|muscle|bulk|mass|hypertrophy|xây dựng cơ/
        )
      ) {
      ...
      if (
        lowerMessage.match(
          /giảm cân|giam can|lose weight|weight loss|cut|fat loss/
        )
      ) {
    ...
    if (
      lowerMessage.match(
        /tăng cơ|tang co|muscle gain|build muscle|bulk|hypertrophy/
      )
    ) {
    ...
    if (
      lowerMessage.match(
        /giảm cân|giam can|lose weight|weight loss|giảm béo|giảm mỡ|cut|deficit|fat loss/
      )
    ) {
      return "workout_planning";
    }
    ...
    if (
      lowerMessage.match(
        /tập luyện|tap luyen|workout|gym|exercise|bài tập|bai tap|training/
      )
    ) {
      if (
        lowerMessage.match(
          /plan|kế hoạch|ke hoach|lịch tập|lich tap|schedule|program/
        )
      ) {
        return "workout_planning";
      }
      if (
        lowerMessage.match(
          /form|kỹ thuật|ky thuat|technique|cách thực hiện|hướng dẫn/
        )
      ) {
    ...
    if (
      lowerMessage.match(
        /an|dinh duong|nutrition|calo|protein|carb|fat|meal|thuc don|thuc an|do an/
      )
    ) {
    ...
    if (
      lowerMessage.match(
        /mệt|tired|lazy|lười|khó khăn|motivation|động lực|không muốn|chán|boring|give up|bỏ cuộc|stress|áp lực/
      )
    ) {
      return "motivation";
    }
    ...
    if (
      lowerMessage.match(
        /tiến độ|progress|kết quả|result|tracking|theo dõi|đo lường/
      )
    ) {
  ...
  private async buildAIContext(params: any): Promise<any> {
    ...
    try {
      ...
      if (intent.includes("workout") || intent.includes("exercise")) {
        ...
        intent.includes("goal") ||
        intent.includes("motivation") ||
        intent.includes("progress")
      ) {
        category = "fitness";
      }
      ...
      if (knowledgeResults.length > 0) {
        ...
        knowledgeResults.forEach((result, idx) => {
          ...
          if (result.subcategory) {
            retrievedKnowledge += `   (${result.category}/${
              result.subcategory
            }, similarity: ${(result.similarity! * 100).toFixed(1)}%)\n`;
          }
        });
        logger.info(
          `✅ Retrieved ${knowledgeResults.length} knowledge items using RAG`
        );
      } else {
        // Fallback: dùng old knowledge base nếu không tìm thấy
        logger.warn("No RAG results found, using fallback knowledge base");
        retrievedKnowledge = knowledgeBase.getRelevantKnowledge(
          message,
          intent
        );
      }
    ...
    if (
      ...
      exerciseData += "\n💡 HƯỚNG DẪN CHO AI:\n";
      exerciseData +=
        "- Hãy giới thiệu những bài tập này một cách chi tiết và hấp dẫn\n";
      exerciseData +=
        "- Đưa ra lời khuyên về cách thực hiện và lưu ý an toàn\n";
      exerciseData += "- Gợi ý số set, rep phù hợp với người dùng\n";
      exerciseData +=
        "- Khuyến khích người dùng nhấn vào bài tập để xem chi tiết\n";
      exerciseData +=
        "- Có thể đề xuất tạo kế hoạch tập luyện với những bài tập này\n\n";
    }
    ...
    if (
      ...
      if (nutritionAnalysis.nutritionContext) {
        ...
        foodData += `- Mục tiêu: ${
          ctx.objective === Objective.LOSE_FAT
            ? "Giảm cân"
            : ctx.objective === Objective.GAIN_MUSCLE
            ? "Tăng cơ"
            : "Duy trì"
        }\n`;
      ...
      if (nutritionAnalysis.mealTime) {
        ...
        foodData += `Bữa ăn: ${
          mealTimeMap[nutritionAnalysis.mealTime] || nutritionAnalysis.mealTime
        }\n`;
      }
      foodData += "\n";
      ...
      nutritionAnalysis.foods.forEach((food: any, index: number) => {
        ...
        if (food.description) {
          foodData += `   - Mô tả: ${food.description.substring(0, 150)}${
            food.description.length > 150 ? "..." : ""
          }\n`;
        }
        if (food.benefits) {
          foodData += `   - Lợi ích: ${food.benefits.substring(0, 150)}${
            food.benefits.length > 150 ? "..." : ""
          }\n`;
        }
        if (food.preparationTips) {
          foodData += `   - Cách chế biến: ${food.preparationTips.substring(
            0,
            100
          )}${food.preparationTips.length > 100 ? "..." : ""}\n`;
        }
      ...
      foodData += "\n💡 HƯỚNG DẪN CHO AI:\n";
      foodData +=
        "- Giới thiệu các thực phẩm này một cách hấp dẫn và tự nhiên\n";
      foodData +=
        "- Giải thích lợi ích dinh dưỡng phù hợp với mục tiêu của người dùng\n";
      ...
      foodData +=
        "- Trả lời tự nhiên, thân thiện như một chuyên gia dinh dưỡng\n\n";
    }
...
Hãy trả lời câu hỏi sau một cách hữu ích và chuyên nghiệp:`;
  ...
  private generateFallbackResponse(message: string, intent: string): any {
    const responses: any = {
      ...
      nutrition_planning: {
        content:
          "Dinh dưỡng rất quan trọng! Hãy chia sẻ mục tiêu và sở thích ăn uống để tôi tư vấn phù hợp.",
        suggestions: [
          "Tăng cơ nutrition",
          "Giảm mỡ nutrition",
          "Tính TDEE",
  ...
  private generateSuggestions(message: string): string[] {
    ...
    if (
      lowerMessage.includes("tập luyện") ||
  ...
  private getConversationFromMemory(
    conversationId: string
  ): ConversationMessage[] {
...
English:`;
      ...
      logger.warn(
        `Translation failed for "${query}", using original:`,
        error.message
      );
  ...
  private async analyzeNutritionIntent(
    ...
    try {
      ...
      if (
        ...
        lowerMessage.includes("bữa sáng")
      ) {
        mealTime = "breakfast";
      } else if (
        lowerMessage.includes("trưa") ||
        lowerMessage.includes("lunch") ||
        lowerMessage.includes("bữa trưa")
      ) {
        mealTime = "lunch";
      } else if (
        lowerMessage.includes("tối") ||
        lowerMessage.includes("dinner") ||
        lowerMessage.includes("bữa tối")
      ) {
        mealTime = "dinner";
      } else if (
        lowerMessage.includes("snack") ||
        lowerMessage.includes("ăn vặt")
      ) {
        mealTime = "snack";
      }
      ...
      if (userProfile) {
        try {
          ...
          const goal =
            fitnessGoals && fitnessGoals.length > 0 ? fitnessGoals[0] : null;
          ...
          if (
            ...
            const nutritionTarget =
              this.nutritionCalculator.calculateNutritionTarget(
                {
                  age: userProfile.age,
            ...
            logger.info(
              `📊 Calculated nutrition context: ${nutritionContext.targetCalories} cal, ${nutritionContext.targetProtein}g protein`
            );
          }
        } catch (error) {
      ...
      try {
        ...
        if (mealTime && nutritionContext) {
          ...
          const mealCalories = Math.round(
            nutritionContext.targetCalories * 0.3
          );
          searchQuery = `Gợi ý món ăn cho ${
            mealTimeMap[mealTime] || mealTime
          } với khoảng ${mealCalories} calories`;

          if (nutritionContext.objective === Objective.GAIN_MUSCLE) {
            const mealProtein = Math.round(
              nutritionContext.targetProtein * 0.3
            );
        ...
        logger.info(
          `🔍 Searching foods for query: "${searchQuery}" (original: "${message}")`
        );
        ...
        if (nutritionContext) {
          ...
          filters.maxCalories = Math.round(
            nutritionContext.targetCalories * 0.3
          );
        ...
        logger.info(
          `🔍 Initial search returned ${foodResults?.length || 0} foods`
        );
        ...
        if (finalResults.length === 0 && Object.keys(filters).length > 0) {
          logger.warn(
            "⚠️ No foods found with filters, trying without filters..."
          );
          const foodResultsNoFilter =
            await foodVectorService.searchFoodsByVector(
              queryEmbedding,
              {}, // No filters
              [],
              searchLimit
          ...
          if (foodResultsNoFilter && foodResultsNoFilter.length > 0) {
            ...
            logger.info(
              `✅ Found ${finalResults.length} foods without filters`
            );
          }
        }
        ...
        if (finalResults && finalResults.length > 0) {
          ...
          logger.info(
            `✅ Found ${foods.length} relevant foods from database (after sorting)`
          );
        } else {
   ...
   */
  private async analyzeExerciseIntent(
    message: string
  ): Promise<ExerciseAnalysis | null> {
    try {
      ...
      if (targetMuscleGroup) {
        logger.info(
          `🔍 Searching exercises for muscle group: ${targetMuscleGroup}`
        );
      }
      ...
      try {
        ...
        const results = await this.pgVector.similaritySearch(
          searchQuery,
          5,
          0.2
        );
        ...
        if (results && results.length > 0) {
          ...
          const exerciseIds = results
            .map((doc: any) => doc.exerciseId)
            .filter(Boolean);

          if (exerciseIds.length > 0) {
            // Fetch full exercise details from database
            const fullExercises = await this.pgVector.getExercisesByIds(
              exerciseIds
            );
            ...
            logger.info(
              `✅ Found ${exercises.length} exercises using pgVector + database`
            );
          }
        }
      } catch (error: any) {
      ...
      general: [
        {
          ...
          reps_recommended: "10-15",
          instructions:
            "Squat, chống tay xuống, kick chân ra sau, push-up, nhảy lên",
        },
      ],
    };
