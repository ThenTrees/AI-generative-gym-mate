...
export class MealRecommendationService {
  ...
  async generateMealRecommendations(
    ...
    let topRecommendations = sortedRecommendations.slice(
      0,
      NUTRITION_CONSTANTS.MAX_RECOMMENDATIONS
    );

    // ✅ NEW: Giới hạn số lượng mỗi category để tránh quá nhiều cùng loại
    topRecommendations = this.enforceCategoryLimits(
      topRecommendations,
      context
    );
    ...
    const missingMustHave = mustHaveCategories.filter(
      (cat) => !categoryCount[cat] || categoryCount[cat] === 0
    );
    const missingNiceToHave = niceToHaveCategories.filter(
      (cat) => !categoryCount[cat] || categoryCount[cat] === 0
    );
    ...
    const missingCategories =
      missingMustHave.length > 0
        ? missingMustHave
        : missingNiceToHave.length >= 2
        ? missingNiceToHave.slice(0, 2)
        : [];

    if (missingCategories.length > 0) {
      logger.info(
        `🔍 Missing categories for ${
          context.mealTime.code
      ...
      if (additionalFoods.length > 0) {
        ...
        const finalRecommendations = topRecommendations.slice(
          0,
          NUTRITION_CONSTANTS.MAX_RECOMMENDATIONS - replaceCount
        );
        ...
        logger.info(
          `✅ Replaced ${replaceCount} foods with missing categories`
        );

        // ✅ Re-enforce category limits after adding new foods
        const finalWithLimits = this.enforceCategoryLimits(
        ...
        logger.warn(
          `⚠️ Could not find missing categories: ${missingCategories.join(
            ", "
          )}`
        );
      }
    ...
    if (
      context.mealTime.code === "lunch" ||
      context.mealTime.code === "dinner"
    ) {
      // topRecommendations đã được define ở trên
      const hasVegetable = topRecommendations.some((food) =>
      ...
      if (!hasVegetable) {
        ...
        if (vegetableNuts.length === 0) {
          try {
            logger.info(
              `🔍 No vegetables/nuts found in top results for ${context.mealTime.code}, searching separately...`
            );
            ...
            let vegetableEmbedding = await this.pgVectorService.embed(
              vegetableQuery
            );

            let vegetableCandidates =
              await this.foodVectorService.searchFoodsByVector(
                vegetableEmbedding,
                {
                  mealTime: context.mealTime.code,
            ...
            if (!vegetableCandidates || vegetableCandidates.length === 0) {
              logger.warn(
                `No vegetables found with category filter, trying without category filter...`
              );
              vegetableCandidates =
                await this.foodVectorService.searchFoodsByVector(
                  vegetableEmbedding,
              ...
              if (vegetableCandidates) {
                vegetableCandidates = vegetableCandidates.filter(
                  (food: Food) => {
              ...
              if (!vegetableCandidates || vegetableCandidates.length === 0) {
                logger.warn(
                  `No vegetables found with mealTime filter, trying without mealTime filter...`
                );
                vegetableCandidates =
                  await this.foodVectorService.searchFoodsByVector(
                    vegetableEmbedding,
            ...
            if (vegetableCandidates && vegetableCandidates.length > 0) {
              logger.info(
                `✅ Found ${vegetableCandidates.length} vegetable candidates`
              );
              // Score và add vào vegetableNuts
              const scoredVegetables = vegetableCandidates
                .map((food: Food) => {
                  const score = this.calculateFoodScore(food, context);
                  return {
                    ...food,
                    ...
                    servingSuggestion: this.calculateServingSuggestion(
                      food,
                      context.targetCalories
                    ),
                    targetCalories: maxCalories,
              ...
              logger.warn(
                `⚠️ No vegetables found even without category filter`
              );
            }
            ...
            if (vegetableNuts.length < 2) {
              ...
              if (nutCandidates && nutCandidates.length > 0) {
                ...
                nutCandidates = nutCandidates.filter(
                  (food: Food) =>
                    this.isVegetableOrNut(food) &&
                    !vegetableNuts.some(
                      (v) => this.getFoodId(v) === this.getFoodId(food)
                    )
                );

                if (nutCandidates.length > 0) {
                  logger.info(
                    `✅ Found ${nutCandidates.length} nut/dairy candidates`
                  );
                  const scoredNuts = nutCandidates
                    .map((food: Food) => {
                      const score = this.calculateFoodScore(food, context);
                      const foodName = (
                        food.foodNameVi ||
                  ...
                  vegetableNuts.push(
                    ...scoredNuts.slice(0, 2 - vegetableNuts.length)
                  );
                }
              }
            }
            ...
            logger.info(
              `✅ Total vegetables/nuts found: ${vegetableNuts.length}`
            );
          } catch (error) {
            logger.error(
              "Error searching for vegetables/nuts separately:",
              error
            );
          }
        }
        ...
        const vegetableCount = topRecommendations.filter((food) =>
          this.isVegetableOrNut(food)
        ).length;
        if (vegetableCount === 1) {
            ...
            .filter(
              (food) =>
                ...
                !topRecommendations.some(
                  (r) => this.getFoodId(r) === this.getFoodId(food)
                )
            )
            .slice(0, 1);
    ...
    return sortedRecommendations.slice(
      0,
      NUTRITION_CONSTANTS.MAX_RECOMMENDATIONS
    );
  }
  ...
  private enforceCategoryLimits(
    ...
    if (removed.length > 0) {
      logger.info(
        `⚠️ Removed ${removed.length} items due to category limits for ${context.mealTime.code}`
      );
    ...
    if (
      result.length < NUTRITION_CONSTANTS.MAX_RECOMMENDATIONS &&
      removed.length > 0
    ) {
      ...
      const addedCategories = new Set(
        result.map((f) => this.getFoodCategory(f))
      );
      ...
      const added =
        result.length - (NUTRITION_CONSTANTS.MAX_RECOMMENDATIONS - needed);
      if (added > 0) {
        logger.info(
          `✅ Filled ${added} slots from removed items (respecting category limits)`
        );
      }
    }
  ...
  private getFoodCategory(food: any): string {
    ...
    if (
      foodName.includes("thịt") ||
      foodName.includes("cá") ||
      foodName.includes("tôm") ||
      foodName.includes("trứng") ||
      foodName.includes("chicken") ||
      foodName.includes("beef") ||
      foodName.includes("fish") ||
      foodName.includes("shrimp")
    ) {
      return "protein";
    }
    ...
    if (
      ...
      foodName.includes("bánh mì") ||
      foodName.includes("khoai") ||
      foodName.includes("rice") ||
      foodName.includes("bread") ||
      foodName.includes("pasta") ||
      foodName.includes("noodle") ||
      foodName.includes("gạo") ||
      foodName.includes("bún") ||
      foodName.includes("phở") ||
      foodName.includes("miến") ||
    ...
    if (
      ...
      foodName.includes("bắp cải") ||
      foodName.includes("cải bó xôi")
    ) {
      return "vegetables";
    }

    if (
      foodName.includes("trái cây") ||
      foodName.includes("chuối") ||
      foodName.includes("táo") ||
      foodName.includes("cam") ||
      foodName.includes("banana") ||
      foodName.includes("apple") ||
      foodName.includes("orange") ||
      foodName.includes("dâu")
    ) {
      return "fruits";
    }
    ...
    if (
      ...
      foodName.includes("bơ") ||
      foodName.includes("dầu") ||
      foodName.includes("nut") ||
      foodName.includes("avocado")
    ) {
      return "fats";
    }
  ...
  private normalizeCategory(category: string): string {
    ...
    if (
      ["carb", "carbs", "carbohydrate", "carbohydrates", "tinh bột"].includes(
        cat
      )
    )
      return "carbs";
    if (["vegetable", "vegetables", "rau", "rau củ", "rau quả"].includes(cat))
      return "vegetables";
    if (["fruit", "fruits", "trái cây", "hoa quả"].includes(cat))
      return "fruits";
  ...
  private async searchMissingCategories(
    ...
    for (const category of missingCategories) {
      try {
        ...
        if (
          ...
          (category === "vegetables" || category === "carbs")
        ) {
          logger.info(
            `⚠️ No ${category} found with category filter, trying without category filter...`
          );
          ...
          logger.info(
            `🔍 Found ${
              candidates?.length || 0
            } candidates without category filter`
          );
        }
        ...
        if (candidates && candidates.length > 0) {
          ...
          logger.info(
            `🔍 Sample candidates for ${category}: ${JSON.stringify(
              sampleCandidates
            )}`
          );
          ...
          const filtered = candidates.filter((food: Food) => {
            ...
            if (
              ...
              existingRecommendations.some(
                (r) => this.getFoodId(r) === candidateId
              )
            ) {
              return false;
            }
            ...
            if (!matches && candidates.indexOf(food) < 5) {
              ...
              logger.info(
                `❌ Mismatch: "${
                  ...
                  food.category
                }, detected: ${detectedCategory}, looking for: ${category})`
              );
            }
          ...
          logger.info(
            `📊 Filtered ${filtered.length} ${category} items from ${candidates.length} candidates`
          );
          ...
          if (filtered.length > 0) {
            ...
            logger.info(
              `✅ Found ${category} candidate: ${
                scored[0].foodNameVi || scored[0].foodName
              }`
            );
          } else {
            logger.warn(
              `⚠️ No ${category} found after filtering (had ${
                candidates?.length || 0
              } candidates)`
            );
          }
    ...
    logger.info(
      `📦 Total additional foods found: ${
        additionalFoods.length
      } for categories: ${missingCategories.join(", ")}`
    );
