...
import {
  ...
  SearchGymsParams,
  SaveLocationParams,
} from "../types/model/gymLocation.model";
...
class LocationService {
  ...
  constructor() {
    this.apiKey =
      process.env.GEOAPIFY_API_KEY || "fddfbe2852db4c35bb0289b2cbf316be";
    this.baseUrl = "https://api.geoapify.com/v2";

    if (!this.apiKey) {
      logger.warn(
        "Geoapify API key not found. Location services will use mock data."
      );
    } else {
  ...
  async searchNearbyGyms({
    ...
    radius = 10000,
    type = "gym",
  }: SearchGymsParams): Promise<GymLocation[]> {
    try {
      if (!this.apiKey) {
      ...
      logger.info(
        `Found ${gyms.length} gyms near ${latitude}, ${longitude} using Geoapify`
      );
      return gyms;
    } catch (error: any) {
   ...
   */
  private formatGymLocationFromGeoapify(
    place: any,
    userLat: number,
    userLng: number
  ): GymLocation {
  ...
  private formatOpeningHoursFromGeoapify(properties: any): {
    today: string;
    week: string[];
  } {
    return {
      today: "6:00 - 22:00",
      week: ["T2-T6: 6:00 - 22:00", "T7-CN: 7:00 - 21:00"],
    };
  }
    ...
    return [
      {
        ...
        openingHours: {
          today: "5:30 - 22:30",
          week: ["T2-T6: 5:30 - 22:30", "T7-CN: 6:00 - 21:00"],
        },
        features: [
          "Pool",
          "Yoga",
      ...
      },
      {
        ...
        openingHours: {
          today: "6:00 - 22:00",
          week: ["T2-T7: 6:00 - 22:00", "CN: 7:00 - 20:00"],
        },
